...
b.count <- ngram_compute(train[1:M],n=2)   # Bigram model
t.count <- ngram_compute(train[1:M],n=3)   # Trigram model
save(u.count,b.count,t.count,file="Model_All.RData")

# u.model <- data.frame(w1 = names(u.count), count = u.count, row.names = NULL)
# 
# PQ <- unlist(strsplit(names(b.count),split = " ")); L <- length(PQ)
# b.model <- data.frame(w1 = PQ[seq(1,(L-1),by=2)], w2 = PQ[seq(2,L,by=2)], count = b.count, row.names = NULL)
# 
# PQR <- unlist(strsplit(names(t.count),split = " ")); L <- length(PQR)
# t.model <- data.frame(w1 = PQR[seq(1,(L-2),by=3)], w2 = PQR[seq(2,(L-1),by=3)], w3 = PQR[seq(3,L,by=3)],
#                       count = t.count, row.names = NULL)
# 
...
u.model <- rbind(as.matrix(u.count),"UNK" = 0) + 1 # Laplacian smoothing
...
ngram_predict <- function(data) {
    ...
    if(L>3) {
        # Bigram model
        bm.mle <- bigram_mle(D,u.model,b.model,1)
        bm.word <- rownames(u.model)[which.max(bm.mle)]
             
        # Trigram model
        tm.mle <- trigram_mle(D,u.model,b.model,t.model,1)
        tm.word <- rownames(u.model)[which.max(tm.mle)]
        
        # Back off approach
        tbu.mle <- backoff_mle(D,u.model,b.model,t.model,1)
