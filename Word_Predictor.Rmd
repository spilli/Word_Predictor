...
data.blogs <- ngram_tokenize("final/en_US/en_US.blogs.txt",50000)
data.news <- ngram_tokenize("final/en_US/en_US.news.txt",50000)
data.twitter <- ngram_tokenize("final/en_US/en_US.twitter.txt",50000)
```
...
```
```{r,echo=FALSE}
lenread <- c(length(data.blogs),length(data.news),length(data.twitter))
...
x <- data.frame(FileName = c(as.character(a$Filename[1:3]),"Total"),
                LenRead = prettyNum(c(lenread,sum(lenread)),big.mark = ","),
    ...
    for (i in 1:ns) {
        ...
        if(n==2) { # Bigrams
            ...
            train_set <- unlist(lapply(train_set[as.logical(idx2)],function(x) {
                mapply(function(y,z) { paste(y,z,collapse = " ") },x[1:(length(x)-1)],x[2:length(x)], USE.NAMES = FALSE)
            }))
        }
        if(n==3) { # Trigrams
            train_set <- sapply(train_set,function(x) x[x %in% dictionary]) # select words in dictionary
            ...
            train_set <- unlist(lapply(train_set[as.logical(idx3)],function(x) {
                mapply(function(p,q,r) {paste(p,q,r,collapse = " ")},x[1:(length(x)-2)],x[2:(length(x)-1)],x[3:length(x)], USE.NAMES = FALSE)
            }))
        }
...
library(wordcloud)
um <- sort(apply(unigrams_without_stopwords,1,sum,na.rm=TRUE),decreasing = TRUE)
round(um[1:10],1) 
wordcloud(names(um),um,max.words=30, scale = c(4,1),rot.per = 0.2, colors = brewer.pal(6,"Dark2"))
```

**Top 10 unigrams with stop words and their average counts are**
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
um <- sort(apply(unigrams,1,sum,na.rm=TRUE), decreasing = TRUE)
round(um[1:10],1) 
wordcloud(names(um),um,max.words=150,scale = c(4,1),rot.per = 0.2, colors = brewer.pal(6,"Dark2"))
```

```{r,fig.width=10,fig.height=6,fig.align="center"}
par(mfrow=c(1,2),mar=c(5,5,2,2))
boxplot(t(unigrams[10:1,]),xlab = "frequency (Number of unigrams per 100 lines)", 
        main = "Unigram frequencies",horizontal = T,las=1)
boxplot(t(unigrams_without_stopwords[10:1,]),xlab = "frequency (Number of unigrams per 100 lines)", 
...
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
bm <- sort(apply(bigrams,1,sum,na.rm=TRUE),decreasing = TRUE)
round(bm[1:10],1) 
wordcloud(names(bm),bm,max.words=100,scale=c(4,0.5),rot.per = 0.2, colors = brewer.pal(6,"Dark2"))
```

**The top 10 trigrams and their total counts are**
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
tm <- sort(apply(trigrams,1,sum,na.rm=TRUE),decreasing = TRUE)
round(tm[1:10],1) 
wordcloud(names(tm),tm,max.words=30,scale=c(4,0.2),rot.per = 0.3, colors = brewer.pal(6,"Dark2"))
```

```{r,fig.width=10,fig.height=6,fig.align="center"}
par(mfrow = c(1,2), mar=c(4,6,2,2))
boxplot(t(bigrams[10:1,]),xlab = "frequency (of 100 lines)", main = "Bigram frequencies",horizontal = T,las=1)
boxplot(t(trigrams[10:1,]),xlab = "frequency (of 100 lines)", main = "Trigram frequencies",horizontal = T,las=1)
```
...
names(x) <- c("N-gram", "Num ngrams in Entire data read(150k likes)", "Num ngrams in Training set(90k lines)", "Num ngrams in Sampled Training set(10k lines)", "Num Unique ngrams", "Num Unique Selected ngrams")
kable(x)
```
...
The number of tokens in the model **N = 771,345** and the number of words in the dictionary **V = 8,000**.
...
x <- data.frame("Unigrams" = names(u.logpr[idu[1:10]]), 
                "Unigram Counts" = u.csum[idu[1:10]], 
                "Unigram log prob" = round(u.logpr[idu[1:10]],2),
                "Bigrams" = names(b.logpr[idb[1:10]]), 
                "Bigram Counts" = b.csum[idb[1:10]],
                "Bigram log prob" = round(b.logpr[idb[1:10]],2),
                "Trigrams" = names(t.logpr[idt[1:10]]), 
                "Trigram Counts" = t.csum[idt[1:10]],
                "Trigram log prob" = round(t.logpr[idt[1:10]],2), row.names = NULL)
kable(x,align="c")
```
...
u_perp <- ngram_perplexity(val_set,u.count,b.count,u.logpr,b.logpr,t.logpr,1) # Unigram model perplexity
b_perp <- ngram_perplexity(val_set,u.count,b.count,u.logpr,b.logpr,t.logpr,2) # Bigram model perplexity
...
From the perplexity numbers, the model of choice would be **bigram model - with smoothing and backoff** as it has the least perplexity on validation data set.
...
```{r}
ngram_predict(tolower(unlist(strsplit("I Love",split = " "))))
ngram_predict(tolower(unlist(strsplit("This looks",split = " "))))
ngram_predict(tolower(unlist(strsplit("This looks like",split = " "))))
ngram_predict(tolower(unlist(strsplit("This looks like a",split = " "))))
ngram_predict(tolower(unlist(strsplit("This looks like a lot",split = " "))))
ngram_predict(tolower(unlist(strsplit("of course thank",split = " "))))
```
