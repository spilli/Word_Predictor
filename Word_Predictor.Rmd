...
```{r,echo=FALSE}
ngram_compute <- function(data,n,mat = 0) {
...
```
```{r,echo=FALSE}
# save(um,bm,tm,ums, u.count,b.count,t.count, file="Report.RData")
load("Report.RData")
```
...
```{r}
sum(um); length(um)
round(c(sum(um[1:1500]), sum(um[1:15000]))/sum(um)*100,2)
```

In the test sample set considered with sample size of **100** lines and **100** such samples, there are **10,000** lines of text containig **160,834** words and **30,307** unique unigrams. Data shows that in a frequency sorted word dictionary, only **1500** unique words are neeed to cover about **50%** of all the word instance and only **15,000** unique words (~50% of unigrams) are needed to cover about **90%** of all word instances. Lets consider the top **15,000** unigrams to be part of the dictionary.

```{r}
dictionary <- names(um[1:15000])
```
...
bigrams <- ngram_compute(train[1:M],n=2,mat=1,dictionary)
trigrams <- ngram_compute(train[1:M],n=3,mat=1,dictionary)
...
```{r,echo=TRUE,eval=FALSE}
M <- 1000 # number of lines of text per each iteration
N <- 1000 # number of iterations

# Unigram model
u <- list()
for (i in 1:N) { u[[i]] <- ngram_compute(train[((i-1)*M+1) : (i*M) ],n=1,mat=0,thres=0);cat(" ",i) } # compute
unique_u <- unique(unlist(sapply(u,function(x) names(x))))
u.count <- matrix(0,length(unique_u),1); rownames(u.count) <- unique_u
for(i in 1:length(u)) { u.count[names(u[[i]]),1] <- u.count[names(u[[i]]),1] + u[[i]] } # collate
ix <- order(u.count,decreasing = TRUE); u.count <- as.matrix(u.count[ix,1]); colnames(u.count) <- "count" # sort

dictionary <- rownames(u.count)[1:25000] # pick all unigrams into dictionary

# Bigram model
b <- list()
for (i in 1:N) { b[[i]] <- ngram_compute(train[((i-1)*M+1) : (i*M) ],n=2,mat=0, dictionary,thres=2);cat(" ",i)  } # compute
unique_b <- unique(unlist(sapply(b,function(x) names(x))))
b.count <- matrix(0,length(unique_b),1); rownames(b.count) <- unique_b
for(i in 1:length(b)) { b.count[names(b[[i]]),1] <- b.count[names(b[[i]]),1] + b[[i]] } # collate
ix <- order(b.count,decreasing = TRUE); b.count <- as.matrix(b.count[ix,1]); colnames(b.count) <- "count" # sort

# Trigram model
t <- list()
for (i in 1:N) { t[[i]] <- ngram_compute(train[((i-1)*M+1) : (i*M) ],n=3,mat=0, dictionary,thres=2);cat(" ",i) } # compute
unique_t <- unique(unlist(sapply(t,function(x) names(x))))
t.count <- matrix(0,length(unique_t),1); rownames(t.count) <- unique_t
for(i in 1:length(t)) { t.count[names(t[[i]]),1] <- t.count[names(t[[i]]),1] + t[[i]] } # collate
ix <- order(t.count,decreasing = TRUE); t.count <- as.matrix(t.count[ix,1]); colnames(t.count) <- "count" # sort
```
...
V <- nrow(u.count) # Number of words in the dictionary
u.model <- rbind(u.count,"UNK" = 0) + 1 # Laplacian smoothing
u.model <- cbind(u.model,"MLE" = log( u.model[,"count"] / (N+V) ) )

# Calculate bigram log likelihood
PQ <- rownames(b.count)
P <- sapply(strsplit(PQ, split = " "), function(x) x[1])
bm.1 <- b.count + 1 # Laplacian smoothing
bm.1 <- cbind(bm.1, "MLE" = log(bm.1[,1]/u.model[P,1]) ) # "Word1" "Word2"
bm.2 <- as.matrix(cbind(rep(1,V),log(1/u.model[1:V,1]))) # "Word1" "UNK"
rownames(bm.2) <- sapply(rownames(u.count),function(x) paste(x,"UNK",collapse = " "),USE.NAMES = FALSE)
bm.3 <- as.matrix(cbind(1,u.model["UNK",2])); rownames(bm.3) <- "UNK UNK" # "UNK" "UNK"
b.model <- rbind(bm.1,bm.2,bm.3)

# Calculate trigram log likelihood
PQR <- rownames(t.count)
PQ <- sapply(strsplit(PQR, split = " "), function(x) paste(x[1:2],collapse = " "))
t.model <- cbind("count" = (t.count+1), "MLE" = log((t.count[,1]+1)/b.model[PQ,1]) )

# Save models to disk
save(u.model,b.model,t.model, file="Model.RData")
```
...
backoff_mle <- function(D,u.model,b.model,t.model,flag = 0) {
    L <- length(D)
    if(flag == 0) {
        P <- D[1:(L-2)]; Q <- D[2:(L-1)]; R <- D[3:L]
    }else { # special case for predicted word
    ...
    a <- matrix(NA,length(PQR),1)
    if(sum(id1)>0) { a[id1] <- t.model[PQR[id1],2] }
    if(sum(id2)>0) { a[id2] <- b.model[PQ[id2],2] + b.model[QR[id2],2] + log(2*0.4) }
    if(sum(id3)>0) { a[id3] <- b.model[PQ[id3],2] + u.model[Q[id3],2] + u.model[R[id3],2] + log(3*0.4) }
    if(sum(id4)>0) { a[id4] <- u.model[P[id4],2] + u.model[Q[id4],2] + b.model[QR[id4],2] + log(3*0.4) }
    if(sum(id5)>0) { a[id5] <- u.model[P[id5],2] + u.model[Q[id5],2] + u.model[R[id5],2] + log(4*0.4) }
...
ngram_predict <- function(data) {
    ...
    if(L>3) {
        # Unigram model
        um.mle <- sum(u.model[D,2]) + u.model[1:(nrow(u.model)-1),2]
        um.word <- rownames(u.model)[which.max(um.mle)]
        
        # Bigram model
        b.stat <- bigram_mle(D,u.model,b.model)
        b.dict <- bigram_mle(D,u.model,b.model,1)
        bm.mle <- sum(b.stat) + b.dict
        bm.word <- rownames(u.model)[which.max(bm.mle)]
             
        # Trigram model
        t.stat <- trigram_mle(D,u.model,b.model,t.model)
        t.dict <- trigram_mle(D,u.model,b.model,t.model,1)
        tm.mle <- sum(t.stat) + t.dict
        tm.word <- rownames(u.model)[which.max(tm.mle)]
        
        # Back off approach
        tbu.stat <- backoff_mle(D,u.model,b.model,t.model)
        tbu.dict <- backoff_mle(D,u.model,b.model,t.model,1)
        tbu.mle <- sum(tbu.stat) + tbu.dict
        tbu.word <- rownames(u.model)[which.max(tbu.mle)]
...
for(i in 1:100) {
    ...
    prev <- ifelse(length(data)>3,data[(length(data)-1)],"UNK")
    truth <- data[length(data)]
    pred <- ngram_predict(newdata)
    df <- rbind(df, data.frame(prev = prev, truth = truth, pred = pred))
}
```
...
```{r, eval = FALSE}
ngram_compute <- function(data,n,mat = 1, dictionary=character(0)) {
    data <- ngram_tokenize(data) # Tokenize the data
    # Create n-grams
    if(n==2) { # Bigrams
    ...
    if(mat==1) {
        ...
        data <- unlist(data); ngram <- table(data); ngram <- ngram[ngram > 2] # count and prune
    }
    ngram
}
```
...
trigram_mle <- function(D,u.model,b.model,t.model,flag = 0) {
    ...
    PQ <- paste(P,Q)
    id2 <- (!id1) & (PQ %in% rownames(b.model))
    id3 <- (!id1) & (!(PQ %in% rownames(b.model)))
...
### backoff_mle()
Function to compute MLE when backoff approach is used. It can also compute MLE against a dictionary while predicting the next word
```{r,eval=FALSE}
backoff_mle <- function(D,u.model,b.model,t.model,flag = 0) {
    L <- length(D)
    if(flag == 0) {
