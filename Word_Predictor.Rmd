...
library(wordcloud)
um <- sort(rowMeans(unigrams_without_stopwords,na.rm=TRUE),decreasing = TRUE)
round(um[1:10],1) 
...
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
um <- sort(rowMeans(unigrams,na.rm=TRUE), decreasing = TRUE)
round(um[1:10],1) 
wordcloud(names(um),um,max.words=50,scale = c(5,1),rot.per = 0.2, colors = brewer.pal(6,"Dark2"))
```
...
**The top 10 bigrams and their average counts are**
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
bm <- sort(rowMeans(bigrams,na.rm=TRUE),decreasing = TRUE)
round(bm[1:10],1) 
wordcloud(names(bm),bm,max.words=100,rot.per = 0.2, colors = brewer.pal(6,"Dark2"))
```

**The top 10 trigrams and their average counts are**
```{r,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE}
tm <- sort(rowMeans(trigrams,na.rm=TRUE),decreasing = TRUE)
round(tm[1:10],1) 
wordcloud(names(tm),tm,max.words=100,scale=c(2,0.1),rot.per = 0.3, colors = brewer.pal(6,"Dark2"))
```
...
The number of tokens in the model **N = 859,709** and the number of words in the dictionary **V = 8000**.
