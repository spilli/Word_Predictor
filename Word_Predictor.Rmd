...
```{r,echo=FALSE}
ngram_compute <- function(data,n,dictionary=character(0),mat = 0) {
    L <- length(data)
    ...
    if(mat==1) {
        ngram <- matrix(0,length(unique_ngrams),L)
        rownames(ngram) <- unique_ngrams; colnames(ngram) <- 1:L
        for(i in 1:L) { 
            ns <- table(data[[i]])
            ngram[names(ns),i] <- ns 
        }
    }else {
        ngram <- matrix(0,length(unique_ngrams),1); rownames(ngram) <- unique_ngrams;
        data <- unlist(data)
        for(i in 1:L) { ngram[i] <- sum(data == unique_ngrams[i]) }
    }
...
um2 <- um2_all[1:15000]
dictionary <- names(um2_all[1:15000]) # pick a dictionary without stop words
```
```{r,echo=FALSE}
...
## Model Building 
Unigram, Bigram and Trigram models built on sampled sets are accummulated and model maximum log likelihoods are calculated. The following options are considered in model building.
...
2. **Stop words** : Stop words are kept in unigram model -------
...
ngram_predict <- function(data) {
    ...
    # Unigram model
    um.mle <- sum(um1.mat[D,2]) + um1.mat[1:(nrow(um1.mat)-1),2]
    ...
    pred <- rbind(um.mle,bm.mle,tm.mle)
    rownames(pred[which.max(pred),])
...
### trigram_mle()
Function to compute trgram MLE of a test data set. It can also compute MLE against a dictionary which can be used to predict the next word.
```{r,eval=FALSE}
trigram_mle <- function(D,um1.mat,bm.mat,tm.mat,flag = 0) {
    D[!(D %in% rownames(um1.mat))] <- "UNK"; L <- length(D)
    if(flag == 0) {
